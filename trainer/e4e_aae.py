...
import lpips
import math
from external.e4e.models.encoders.psp_encoders import ProgressiveStage


class E4eAaeTrainer(StyleGanTrainer):
    def run_batch(self, batch, batch_idx, cur_step):
        self.check_for_progressive_training_update(batch_idx)
    ...
    def check_for_progressive_training_update(self, batch_idx):
        stylegan_size = self.cfg.dataset.imsize
        log_size = int(math.log(stylegan_size, 2))
        num_style_layers = 2*log_size - 2
        num_deltas = num_style_layers - 1
        progressive_steps = [0]
        next_progressive_step = 20000
        for i in range(num_deltas):
            progressive_steps.append(next_progressive_step)
            next_progressive_step += 2000

        for i in range(len(progressive_steps)):
            if batch_idx == progressive_steps[i]:
                self.G.e.set_progressive_stage(i)
                self.G_ema.e.set_progressive_stage(i)
                self.ddp_modules['G_e'].set_progressive_stage(i)
                print(f'[INFO] updating progressive stage to {i}')


    def run_G(self, real, do_extra_losses, sync):
        ...
        with misc.ddp_sync(G_e, sync):
            ...
            if self.delta_loss_enabled and do_extra_losses:
                total_delta_loss = 0
                deltas_latent_dims = G_e.get_deltas_starting_dimensions()
                first_w = ws[:, 0, :]
                for i in range(1, G_e.progressive_stage.value + 1):
                    curr_dim = deltas_latent_dims[i]
                    delta = ws[:, curr_dim, :] - first_w
                    delta_loss = torch.norm(delta, 2, dim=1).mean()
                    total_delta_loss += delta_loss
            else:
                total_delta_loss = None

        # synthesize
        with misc.ddp_sync(G_g, sync):
            output = G_g(ws)

        if not do_extra_losses:
            return output

        # face loss
        with misc.ddp_sync(face_loss_model, sync):
            face_loss = face_loss_model(output, real.detach())

        # perceptual loss
        with misc.ddp_sync(perceptual_loss_model, sync):
            perceptual_loss = perceptual_loss_model(output, real.detach())

        return output, total_delta_loss, face_loss, perceptual_loss
    ...
    def _init_modules(self):
        ...
        # delta loss
        self.delta_loss_enabled = cfg.loss.G.delta.enabled
