...
class BrewLoss:
    ...
    def accumulate_gradients(self, phase, w, gender, sync, gain):
        ...
        if phase == 'G_aae':
            ...
            with torch.autograd.profiler.record_function('G_rec_forward'):
                # recreate image
                pixel_loss, face_loss, perceptual_loss = self.trainer.run_G(
                    w=w.detach(),
                    gender=gender.detach(),
                    do_rec_losses=True,
                    sync=sync,
        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                target = self.trainer.get_target_img(w, gender, sync=False)
    ...
    def accumulate_gradients(self, phase, w, gender, sync, gain):
        ...
        with torch.autograd.profiler.record_function('main_forward'):
            ...
            if perceptual_loss is not None:
                training_stats.report('Loss/G/perceptual', perceptual_loss)
                total_loss += perceptual_loss * self.perceptual_weight
            if face_loss is not None:
                training_stats.report('Loss/G/face', face_loss)
                total_loss += face_loss * self.face_weight
