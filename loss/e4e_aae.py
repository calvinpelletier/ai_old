    ...
    def accumulate_gradients(self, phase, real, sync, gain):
        ...
        if phase == 'G_aae':
            ...
            with torch.autograd.profiler.record_function('G_rec_forward'):
                # recreate image
                rec, delta_loss, face_loss, perceptual_loss = self.trainer.run_G(
                    real=real.detach(),
                    do_extra_losses=True,
                    sync=sync,
                )
                ...
                if self.delta_loss_enabled:
                    training_stats.report('loss/G/delta', delta_loss)
                    rec_loss += delta_loss * self.delta_weight
