...
class StyleGanLoss:
    ...
    def accumulate_gradients(self, phase, real_img, gen_z, sync, gain):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws = self.trainer.run_G(
                    gen_z,
                    sync=(sync and not do_Gpl),
                )
                gen_logits = self.trainer.run_D(gen_img, sync=False)
        ...
        if do_Gpl:
            with torch.autograd.profiler.record_function('Gpl_forward'):
                ...
                gen_img, gen_ws = self.trainer.run_G(
                    gen_z[:batch_size],
                    sync=sync,
                )
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, _gen_ws = self.trainer.run_G(gen_z, sync=False)
...
class BlendStyleGanLoss:
    ...
    def accumulate_gradients(self, phase, real_img, ibg, gen_z, sync, gain):
        assert phase in ['Gmain', 'Greg', 'Gboth', 'Dmain', 'Dreg', 'Dboth']
        do_Gmain = (phase in ['Gmain', 'Gboth'])
        do_Dmain = (phase in ['Dmain', 'Dboth'])
        do_Gpl   = (phase in ['Greg', 'Gboth']) and (self.pl_weight != 0)
