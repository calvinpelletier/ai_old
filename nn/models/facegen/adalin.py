...
from ai_old.nn.blocks.adalin import UpAdalinBlock, NonResAdalinBlock
from ai_old.util.etc import log2_diff
from ai_old.nn.blocks.blend import BlendBlock


class BlendAdalinModulatedGenerator(Unit):
    def __init__(self,
        output_imsize=128,
        init_imsize=4,
        ...
        # initial
        self.initial = NonResAdalinBlock(
            nc[-1],
            nc[-1],
            nc[-1],
            z_dims,
        ...
        for i in range(n_down_up):
            up_blocks.append(UpAdalinBlock(
                nc[i+1],
                nc[i],
                z_dims,
                norm=norm,
...
class AdalinModulatedGenerator(Unit):
    def __init__(self,
        output_imsize=128,
        init_imsize=4,
        nc_in=3,
